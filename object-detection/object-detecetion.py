...
def fileArray(detector):
    ...
    size = len(input_image_list)
    for i in range(size):
        input_image_path = "./pic/input/" + input_image_list[i]
        imgArray = cv2.imread(input_image_path)
...
def main():
    ...
    detections, extract_detected_objects = fileArray(detector)
    # print(detections)
    size = len(detections)
    # for i in range(size):
    #     print('------------------- %d -------------------' % int(i + 1))
